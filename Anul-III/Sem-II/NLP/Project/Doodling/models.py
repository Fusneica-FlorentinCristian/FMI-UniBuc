...
def classifier(X, y, X_validate, y_validate, model, model_type=0):
    ...
    match model:
        ...
        case "Bag":
            clf = BaggingClassifier(base_estimator=svm.SVC(kernel='rbf'), n_estimators=10, random_state=0)
        case "RidgeCV":
    ...
    if model_type == 0:
        ...
        clf.fit(x=X, y=to_keras_labels(y), validation_data=(X_validate, to_keras_labels(y_validate)),
                epochs=epochs, batch_size=batch_size, callbacks=[early_stopping])
        clf.fit(x=X, y=np.array(y,
                                dtype='float32'), validation_data=(X_validate, np.array(y_validate, dtype='float32')),
                epochs=epochs,
                batch_size=batch_size, callbacks=[early_stopping])
        # metrics_clf = clf.fit(x=X, y=to_keras_labels(y), validation_data=(X_validate, to_keras_labels(y_validate)),
        #                       epochs=epochs, batch_size=batch_size, callbacks=[early_stopping])
...
def get_encoder_model(DROPOUT_RATE=0.003, no_of_labels=NUMBER_OF_LABELS, width=WIDTH_IMAGE, height=HEIGHT_IMAGE,
                      channels=CHANNELS):
...
def metrics_data(classes, predicted_classes, DEBUG=True):
    if DEBUG:
        print(metrics.classification_report(classes, predicted_classes, labels=np.unique(predicted_classes)))
    return metrics.classification_report(classes, predicted_classes, output_dict=True,
                                         labels=np.unique(predicted_classes)), \
           metrics.confusion_matrix(classes, predicted_classes)


def build_models(X_train, y_train, X_test, models, X_validate=[], y_validate=[], DEBUG=True):
    ...
    for model_index in range(len(models)):
        ...
        if isinstance(clf_train[model_index], Sequential) or X_validate == []:
            ...
            predicted_validate.append(clf_train[model_index].predict(X_validate))
            if X_test != X_validate:
                predicted_test.append(clf_train[model_index].predict(X_test))
            else:
                predicted_test.append(predicted_validate[-1])
            local_cr, local_cm = metrics_data(y_validate, predicted_validate[model_index],
                                              DEBUG=DEBUG)  # , list(set(y_train)))
...
if __name__ == "__main__":
    ...
    clf_train, predicted_validate, predicted_test, cm, cr = build_models(X_train, y_train, X_test, models, X_validate,
                                                                         y_validate)
    # print("CONGRATS")
    for model_index in range(len(models)):

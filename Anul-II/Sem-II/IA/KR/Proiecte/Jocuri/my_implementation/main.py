...
class HelperClass:
    ...
    @staticmethod
    def check_final(current_state):  # 7) Stare finala
    ...
    @staticmethod
    def check_exit(event):  # 10) Oprire joc
...
def display_menu(display):  # 1) Deja impl (butoane etc)
    ...
    btn_difficulty = ButtonsGroup(buttons_list=[  # 2) Nivel
...
class State:
    ...
    def moves(self):
        opposite_player = Game.opposite_player(self.active_player)  # for later use :)
        # 5) Mutari calculator + verif mutare
        game_moves_list = self.game.all_possible_moves(active_player=self.active_player, current_state=self)
        games = [game for game in
                 [self.game.copy_game(move, self.active_player) for move in game_moves_list]]
        # print("games in State.moves()")
        for game in games:  # 5) Mutari calculator + verif mutare
        ...
        # 5) Mutari calculator + verif mutare
        states_list = [State(game, opposite_player, self.depth - 1) for game in games]  # 2) Nivel
...
class Game:
    ...
    def __init__(self, debug=False, initialised=False):
        ...
        if not initialised:  # 3) Tabla initiala # 4) Desen tabla
    ...
    def copy_game(self, changes, active_player):
        ...
        game_copy.last_white_move = self.last_white_move if active_player == "black" else changes
        game_copy.last_black_move = self.last_black_move if active_player == "white" else changes
        game_copy.last_move = changes
        game_copy.selected_piece_node = None
        game_copy.unused_white_pieces = self.unused_white_pieces - 1 if active_player == "white" \
            else self.unused_white_pieces
        game_copy.unused_black_pieces = self.unused_black_pieces - 1 if active_player == "black" \
            else self.unused_black_pieces
        # game = Game(initialised=True)
        # game.last_white_move = copy.deepcopy(self.last_white_move) if active_player == "black" else changes
        # game.last_black_move = copy.deepcopy(self.last_black_move) if active_player == "white" else changes
        # game.last_move = changes
        # game.selected_piece_node = None
        # game.unused_whites = copy.deepcopy(self.unused_white_pieces) - 1 if active_player == "white" \
        #     else copy.deepcopy(self.unused_white_pieces)
        # game.unused_blacks = copy.deepcopy(self.unused_black_pieces) - 1 if active_player == "black" \
        #     else copy.deepcopy(self.unused_black_pieces)
        # game.unused_white_pieces = unused_whites
        # game.unused_black_pieces = unused_blacks
        # game.white_morrises = copy.deepcopy(self.white_morrises)
        # game.black_morrises = copy.deepcopy(self.black_morrises)
        # game.white_pieces = copy.deepcopy(self.white_pieces)
        # game.black_pieces = copy.deepcopy(self.black_pieces)
        return game_copy

    def all_possible_moves(self, active_player, current_state):
        ...
        moves = []  # tuple made of last position of node, current/changed position and deleted node (=None if N/A)
        for node in current_nodes:  # 5) Mutari calculator + verif mutare
            variants = self.variants_node(node, current_state=current_state, white_pieces=white_nodes,
                                          black_pieces=black_nodes)
            for variant in variants:  # possible ways for a node to move (and possibly delete another node)
                moves.append((node, variant[0], variant[1]))

        for new_piece in range(24):  # all nodes' indexes  # 5) Mutari calculator + verif mutare
            if new_piece not in white_nodes + black_nodes:
                if self.check_new_morris(active_player, new_piece=new_piece, set_new_morrises=False):
                    deletable = self.get_removable_piece(current_state, is_player=False)  # list of deletable pieces
                    if len(deletable) > 0:
                        moves.extend([(None, new_piece, deleted_node) for deleted_node in deletable])
                    else:
                        moves.append((None, new_piece, None))
                else:
                    moves.append((None, new_piece, None))
    ...
    def initialise_game(self):
        pygame.init()  # 3) Tabla initiala  # 4) Desen tabla
        pygame.display.set_caption("Fusneica Florentin-Cristian - Twelve men's morris")
        Game.MIN_PLAYER, algorithm_type, difficulty, play_type = display_menu(self.game_display)  # 1) Deja impl (butoane etc)
        self.display_game()  # 3) Tabla initiala  # 4) Desen tabla
        ...
        current_state = State(game=self, active_player='white', depth=int(difficulty))  # 2) Nivel
        if not Game.debug:
            print("Muta alb")
        return algorithm_type, play_type, current_state  # 14) Bonus optiuni
    ...
    def check_new_morris(self, active_player, new_piece=None, set_new_morrises=True, opposite=False):
        ...
        for new_morris in new_morrises:
            if (new_morris not in self.white_morrises and active_player == "white") or (
                    new_morris not in self.black_morrises and active_player == "black"):
                is_new_morris = True
                break
    ...
    def player_move(self, current_state):
        for event in pygame.event.get():
            HelperClass.check_exit(event)  # 10) Oprire joc
                    ...
                    if HelperClass.euclidian_distance(pos, node) <= Graph.pct_radius * 2:
                        ...
                        if node not in self.white_pieces + self.black_pieces:
                            if self.selected_piece_node:
                                ...
                                if (n0, n1) in Graph.edges or (n1, n0) in Graph.edges:  # 6) Mutare utilizator in interf grafica
                                    ...
                                    if node != before_last_moved_node or self.selected_piece_node != last_moved_node:  # 6) Mutare utilizator in interf grafica
                                        ...
                                        if current_state.game.last_move[2]:  # if a node was selected to be deleted  # 6) Mutare utilizator in interf grafica
                                            opposite_pieces.remove(current_state.game.last_move[2])

                                        current_state.game.display_game()
                                        if HelperClass.check_final(current_state):
                                            return "final"
                                        current_state.active_player = self.opposite_player(current_state.active_player)  # 14) Bonus optiuni
                                        HelperClass.print_current_player(current_state.active_player)
                                        self.selected_piece_node = False
                                        return False
                            elif current_state.active_player == "white" and current_state.game.unused_white_pieces > 0:  # 6) Mutare utilizator in interf grafica
                                current_pieces.append(node)
                                current_state.game.display_game()
                                ...
                                HelperClass.print_current_player(current_state.active_player)
                                return False
                            elif current_state.active_player == "black" and current_state.game.unused_black_pieces > 0:  # 6) Mutare utilizator in interf grafica
                                current_pieces.append(node)
                                current_state.game.display_game()
                                ...
                                HelperClass.print_current_player(current_state.active_player)
                                return False
                        elif node in current_pieces:  # 6) Mutare utilizator in interf grafica
    ...
    def bot_move(self, current_state: State, algorithm_type="minmax"):
        ...
        current_state.game = copy.deepcopy(new_state.chosen_state.game)  # 14) Bonus optiuni
        current_state.game.display_game()  # 14) Bonus optiuni
    ...
    def player_vs_bot(self, algorithm_type, current_state):
        while True:
            if current_state.active_player == Game.MIN_PLAYER:
                if self.player_move(current_state):
                    break
            elif current_state.active_player == Game.MAX_PLAYER:
                if self.bot_move(current_state, algorithm_type):
                    break

    def activate_display(self):
        algorithm_type, play_type, current_state = self.initialise_game()
        if play_type == "pvp":  # 14) Bonus optiuni
    ...
    def final_game(self, current_state):  # 7) Stare finala
        ...
        opposite_can_move = self.opposite_player_can_move(current_state)  # 7) Stare finala
...
def min_max(state: State):
    ...
    # 5) Mutari calculator + verif mutare
    state.possible_moves = state.moves()  # a list of States with all possible next moves for current player
    scored_moves = [min_max(move) for move in state.possible_moves]  # every state will receive a score
...
def alpha_beta(alpha, beta, state: State):
    ...
    if state.active_player == Game.MAX_PLAYER:
        ...
        for move in state.possible_moves:
            ...
            if current_score < stare_noua.score:
                state.chosen_state = stare_noua
                current_score = stare_noua.score
...
if __name__ == "__main__":
    ...
    while True:
        for ev in pygame.event.get():
            HelperClass.check_exit(ev)  # 10) Oprire joc

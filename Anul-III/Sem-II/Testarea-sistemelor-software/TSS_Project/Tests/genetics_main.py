...
def basic(target: str, genes: list[str], N_POPULATION, N_SELECTED, MUTATION_PROBABILITY, debug: bool = True) -> tuple[int, int, str]:
    # """
...
if __name__ == "__main__":
    target_str = (
        "This is a genetic algorithm to evaluate, combine, evolve, and mutate a string! Also, TSS is amazing!"
    )
    genes_list = list(
    ...
    # print(basic("doctest", ascii_lowercase, N_POPULATION, N_SELECTED, MUTATION_PROBABILITY, debug=False)[2])
    print(
        "\nGeneration: %s\nTotal Population: %s\nTarget: %s"
        % basic(target_str, genes_list, N_POPULATION, N_SELECTED, MUTATION_PROBABILITY)
    )

...
class Joc:
	...
	def linii_deschise(self, jucator):
		...
		+ self.open_morris(self.matr[1], jucator)
		+ self.open_morris(self.matr[2], jucator)
		+ self.open_morris([self.matr[0][0], self.matr[1][0], self.matr[2][0]], jucator)
		+ self.open_morris([self.matr[0][1], self.matr[1][1], self.matr[2][1]], jucator)
		+ self.open_morris([self.matr[0][2], self.matr[1][2], self.matr[2][2]], jucator)
		+ self.open_morris([self.matr[0][0], self.matr[1][1], self.matr[2][2]], jucator)
		+ self.open_morris([self.matr[0][2], self.matr[1][1], self.matr[2][0]], jucator)
...
def min_max(stare):
	...
	mutariCuEstimare=[min_max(mutare) for mutare in stare.possible_moves]
	


	if stare.active_player==Joc.JMAX :
		#daca jucatorul e MAX_PLAYER aleg starea-fiica cu estimarea maxima
		stare.chosen_state=max(mutariCuEstimare, key=lambda x: x.estimare)
	else:
				...
				if event.type== pygame.QUIT:
					...
					for linie in range(Joc.NR_COLOANE):
						for coloana in range(Joc.NR_COLOANE):
							...
							if Joc.celuleGrid[linie][coloana].collidepoint(pos):#verifica daca punctul cu coord pos se afla in rectangle(celula)
								###############################
								if stare_curenta.tabla_joc.matr[linie][coloana] == Joc.JMIN:
									if (de_mutat and linie==de_mutat[0] and coloana==de_mutat[1]):
										...
										de_mutat=False
        stare_curenta.game.display_game()
									else:
...
stare_curenta.game.display_game()
								elif stare_curenta.game.matr[linie][coloana] == Joc.GOL:
									if de_mutat:
										#### eventuale teste legate de mutarea simbolului
										stare_curenta.game.matr[de_mutat[0]][de_mutat[1]]=Joc.GOL
										de_mutat=False		
									#plasez simbolul pe "tabla de joc"
									stare_curenta.game.matr[linie][coloana]=Joc.JMIN
...
stare_curenta.game.display_game()
									...
									stare_curenta.active_player=Joc.jucator_opus(stare_curenta.active_player)
		...
		#--------------------------------
			...
			stare_curenta.game=stare_actualizata.stare_aleasa.game
